export default function Header({ currentView, onViewChange }) {
  return (
    <header className="site-header no-print">
      <div className="header-container">
        <div className="header-brand">
          <div className="brand-logo-badge">GIMT</div>
          <div className="brand-text">
            <span className="institute-name">Global Institute of Management and Technology</span>
            <span className="department-name">Department of Computer Science & Engineering</span>
          </div>
        </div>

        <div className="header-nav-actions">
          <button
            type="button"
            onClick={() => onViewChange('form')}
            className={`header-nav-btn ${currentView === 'form' ? 'active' : ''}`}
          >
            <svg className="nav-btn-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" />
              <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" />
            </svg>
            <span>Register</span>
          </button>

          
          <div className="header-date-tag">
            3rd Sept 2026
          </div>
        </div>
      </div>
    </header>
  );
}
